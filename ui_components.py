...
UI 組件模組 - 側邊欄、每日聖經金句、神聖公告版、手動控制按鈕

作者：Head Study Prefect 26-27 LI Chuangjie Jacky
版本：v2.3 Final（已整合神聖金句深色漸層、金色文字、全局負荷滑桿、6行多槽位顯示、專業沉穩風格）
"""
...
from core import generate_roster, recommend_substitutes
from utils import (
...
def show_daily_verse():
    ...
    <div style="background: linear-gradient(180deg, #1A1A2E 0%, #0B1E3D 100%);
                padding: 28px 24px; border-radius: 16px; margin: 20px 0 30px 0;
                text-align: center; border: 2px solid #D4AF37; box-shadow: 0 8px 25px rgba(212, 175, 55, 0.15);">
        <h3 style="margin:0 0 12px 0; color:#D4AF37; font-size:18px; letter-spacing:1px;">📖 今日聖經金句</h3>
        <p style="font-size:17px; margin:0; color:#F5E8C7; line-height:1.7; font-weight:500;">
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button("📥 下載名冊格式範例", use_container_width=True):
            ...
            st.download_button("✅ 下載範例檔", output.getvalue(),
                               "Prefect_名冊格式範例.xlsx", use_container_width=True)
        ...
        if st.button("⬇️ 導出完整備份 (JSON)", use_container_width=True):
            ...
            st.download_button("✅ 下載備份檔", backup_json,
...
print("✅ ui_components.py 已載入完成 - 側邊欄、金句顯示、控制按鈕模組就緒")