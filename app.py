...
def get_optimized_substitutes(day, role, absentee, students_df, current_roster):
    ...
    if subs_pool:
        # 最精準優化：依據目前負荷「由輕到重」排序，推薦最輕鬆的前 2~3 名候選人
...
with st.sidebar:
    ...
    sidebar_config = {
        ...
        "available": st.column_config.SelectboxColumn(
            "可當值日子 (多選)", options=DAYS, multiple=True, required=True
        ),
...
if st.session_state.students_df.empty:
    ...
    with col_btn1:
        if st.button("🚀 生成本週值班表", type="primary"):
    ...
    # 3. Markdown (極度方便手機複製轉貼至 WhatsApp 班群)
